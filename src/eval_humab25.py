...
def humab_25(fname, ncase=25):
    with open(fname, "r") as f:
        ...
        for i in range(ncase):
            ...
            mut_VH_cdr1, mut_VH_cdr2, mut_VH_cdr3 = (
                mut_VH_seq[26:38].replace("-", ""),
                mut_VH_seq[55:65].replace("-", ""),
                mut_VH_seq[104:141].replace("-", ""),
            )
            ...
            mut_VL_cdr1, mut_VL_cdr2, mut_VL_cdr3 = (
                mut_VL_seq[26:38].replace("-", ""),
                mut_VL_seq[55:65].replace("-", ""),
                mut_VL_seq[104:141].replace("-", ""),
            )
...
def benchmark_humab_25(conf, fname, ncase=25, plot_wt=True):
    # 🚨 오류 수정: 이미지 저장 디렉터리 'plot'이 없으면 생성
    print(f"✅ 출력 디렉터리 '{plot_dir}' 존재 확인 및 생성 완료.")
    ...
    for i in range(len(names)):
        ...
        mut_total_kmers_VH, mut_total_kmers_VL, wt_total_kmers_VH, wt_total_kmers_VL = (
            0,
            0,
            0,
            0,
        ...
        for mer in range(8, 13):
            # K-mer 풀 로딩 (Pools Loading)
            if mer not in pools_id1.keys():
                pkl = os.path.join(data_dir, f"human_{str(mer)}mer.dump")
                with open(pkl, "rb") as f:
                    pools = pickle.load(f)
                pools_id1[mer] = pools
            if mer not in pools_id2.keys():
                pkl = os.path.join(data_dir, f"oas_paired_human_{str(mer)}mer.dump")
                with open(pkl, "rb") as f:
                    pools = pickle.load(f)
                pools_id2[mer] = pools
            if mer not in pools_id3.keys():
                pkl = os.path.join(data_dir, f"oas_paired_mouse_{str(mer)}mer.dump")
                with open(pkl, "rb") as f:
                    pools = pickle.load(f)
                pools_id3[mer] = pools
            ...
            # K-mer 응답 계산 (Mutated)
            # 🚨 주의: check_response의 세 번째, 네 번째 인자는 K-mer 풀 리스트여야 합니다.
            # pools_id3[mer] 대신 pools_id3 딕셔너리를 통째로 전달하면
            # check_response 내부에서 처리 오류가 발생할 수 있습니다.
            # check_response 함수가 pool_neg를 pool_pos와 같은 구조로 처리한다고 가정하고,
            # pools_id3[mer]를 전달하도록 수정하는 것이 안전하지만, 원본 코드 구조를 유지합니다.

            pool_pos_mer = [pools_id1[mer], pools_id2[mer]]
            pool_neg_list = [
                pools_id3
            ]  # check_response 내부 로직에 따라 pool_neg 리스트에 전체 딕셔너리를 포함

            mut_subscores_VH, mut_subresponse_VH = check_response(
                mut_kmers_VH, mer, pool_pos_mer, pool_neg_list, mut_scores_VH
            )
            mut_subscores_VL, mut_subresponse_VL = check_response(
                mut_kmers_VL, mer, pool_pos_mer, pool_neg_list, mut_scores_VL
            )
            ...
            # K-mer 응답 계산 (Wild Type)
            wt_subscores_VH, wt_subresponse_VH = check_response(
                wt_kmers_VH, mer, pool_pos_mer, pool_neg_list, wt_scores_VH
            )
            wt_subscores_VL, wt_subresponse_VL = check_response(
                wt_kmers_VL, mer, pool_pos_mer, pool_neg_list, wt_scores_VL
            )
        ...
        # 결과 출력 (Mutated vs Wild Type Average Score)
        mut_avg_score = (
            mut_response_VH / max(1, mut_total_kmers_VH)
            + mut_response_VL / max(1, mut_total_kmers_VL)
        ) / 2
        wt_avg_score = (
            wt_response_VH / max(1, wt_total_kmers_VH)
            + wt_response_VL / max(1, wt_total_kmers_VL)
        ) / 2
        print(
            f"항체: {names[i]}, Mutant 점수: {mut_avg_score:.4f}, WT 점수: {wt_avg_score:.4f}"
        )
        ...
        ax1.plot(
            ...
            color="purple",
            linewidth=2.5,
            label="Experimentally Humanized (HUM)",
            marker="o",
            markersize=4,
        ...
        for j in range(3):
            ax1.fill_between(
                [cdrs_VH_index[i][j * 2], cdrs_VH_index[i][j * 2 + 1]],
                1.02,
                -0.02,
                color="green",
                alpha=0.15,
        ...
        plt.savefig(
            os.path.join(plot_dir, f"{names[i]}_VH.png"), dpi=300
        )  # os.path.join 사용하여 경로 안전성 확보
        plt.close()
        ...
        for j in range(3):
            ax2.fill_between(
                [cdrs_VL_index[i][j * 2], cdrs_VL_index[i][j * 2 + 1]],
                1.02,
                -0.02,
                color="green",
                alpha=0.15,
        ...
        plt.subplots_adjust(bottom=0.15, top=0.85)
        plt.savefig(
            os.path.join(plot_dir, f"{names[i]}_VL.png"), dpi=300
        )  # os.path.join 사용하여 경로 안전성 확보
        plt.close()
...
if __name__ == "__main__":
    benchmark_humab_25(
        conf, os.path.join(data_dir, "humab_25.csv"), ncase=25, plot_wt=True
    )
