...
def generate_single_mutation(
    protein_sequence, annotation="", fixed=[]
):  # fixed = [27-31,32,65-68]
...
def plot_ada(VH_score, VL_score, dirname, species):
    ...
    plt.savefig(os.path.join(plot_dir, f"{dirname}_{species}_site.png"), dpi=600)
    plt.close()


def load_kmer_pools(
    min_mer, max_mer, pools_human, pools_oas_pair_human, pools_oas_pair_mouse
):
    """
...
def predict_immunogenicity(seq, min_mer, max_mer):
    ...
    return response / max(1, total_kmers)
