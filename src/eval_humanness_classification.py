...
def benckmark_humanness(conf, pool_pos=[], pool_neg=[], classify=True):
    path = os.path.join(data_dir, "biophi_humanness.csv")
        ...
        for s in seq:
            ...
            for mer in range(conf.min_mer, conf.max_mer):
                ...
                subscores, subresponse = check_response(
                    kmers, mer, pool_mer, neg_pool_mer, scores
                )
...
def main():
    ...
    report = benckmark_humanness(
        conf, pool_pos=pool_pos, pool_neg=pool_neg, classify=True
    )
    return report
